...
public class TrafficLightControllerFase5 : MonoBehaviour
{
    ...
    private IEnumerator ForceRedCoroutine(float duration)
    {
        ...
        // Vai para amarelo primeiro (transição visual)
        SetLightState(LightState.Yellow);
        yield return new WaitForSeconds(yellowTime);

        // Fica vermelho pelo tempo determinado
        SetLightState(LightState.Red);
        yield return new WaitForSeconds(duration);

        // Transição de volta para o ciclo normal: amarelo -> verde
        SetLightState(LightState.Yellow);
        yield return new WaitForSeconds(yellowTime);

        // Reinicia o ciclo do semáforo em verde e reseta o timer
